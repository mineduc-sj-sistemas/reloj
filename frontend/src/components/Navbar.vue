...
import { Fingerprint, Building2, UserPlus, Zap } from 'lucide-vue-next';
...
const emit = defineEmits<{
  (e: 'openDepartment'): void;
  (e: 'openEmployee'): void;
  (e: 'simulatePunch'): void;
}>();
</script>
  ...
  <header class="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-xs">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
      <div class="flex items-center space-x-4">
        <!-- Identificador principal alineado al inicio -->
        <div class="flex items-center gap-2.5">
          <div class="p-2 bg-brand-orange rounded-lg text-white shadow-sm">
            <Fingerprint class="w-6 h-6" />
          </div>
          <span class="text-lg font-black tracking-tight text-black">
        ...
        <img
          ...
          class="h-10 w-auto object-contain border border-slate-200 p-1 rounded-md bg-white shadow-xs hidden sm:block"
        />
      </div>
